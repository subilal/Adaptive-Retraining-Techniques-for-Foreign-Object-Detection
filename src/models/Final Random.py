...
for Split_index in range(num_splits):
    ...
    project = "Output/Adaptive_0_" + str(Split_index)
    batch = 16
    epochs = 2
    image_size = 1024
    device = 1
    ...
    image_path = r"/Train_test_Split_All/DATA/" +  str(Split_index + 1) + "/images"

    all_image_list = os.listdir(image_path)

    number_samples = 16 * 5

    for sam_index in range(number_samples):
        filenamew = choice(all_image_list)
        all_image_list.remove(filenamew)
        shutil.copy(image_path + '/'+filenamew, new_dataset_path )
        labelpath = image_path + '/' + filenamew
        labelpath = labelpath.replace(".bmp",".txt")
        labelpath = labelpath.replace("images","labels")
        shutil.copy(labelpath, new_dataset_path )
    

    if Split_index == 0:
        Split_0_images  = r"Train_test_Split_All/DATA/0/images/"
        Split_0_labels  = r"Train_test_Split_All/DATA/0/labels/"
...
with open('Map_50_all_splits.obj', 'rb') as f:
    x = pickle.load(f)
...
model = YOLO(r'Output/Adaptive_20_8/train4/weights/best.pt')
#result = model.train(data=dataset_yaml, project=project, batch=batch, epochs=epochs, verbose=False, workers=28, single_cls = False,imgsz =  image_size, device = 1 )
...
for x in out_acc:
    upper_limit = round(0.90 + (1.0 - x) / 2,2)
    lower_limit = round(upper_limit - 0.35, 2)
...
for indx in list_cnm:
    print(indx)
    y = x.get_group(indx).sort_values(by ="conf" ).head(5)
    print(y)
